...
Begin
    ...
    Begin
        -- thông báo l?i n?u s?t ?ã t?n t?i
...
Begin
    ...
    End
		...
		BEGIN
			SET @manpp = dbo.TaoMa(N'NhaPhanPhoi'); -- G?i hàm ?? t?o mã m?i
		END
		declare @dauso table (dauso char(3));
		insert into @dauso values   ('086'), ('096'), ('097'), ('098'), ('039'), ('038'), ('037'), 
                                    ('036'), ('035'), ('034'), ('033'), ('032'), ('091'), ('094'), 
...
AS
BEGIN
    -- Tắt thông báo lỗi SQL chi tiết để tránh lộ thông tin nhạy cảm
    SET NOCOUNT ON;
	OPEN SYMMETRIC KEY SymKey_test 
	DECRYPTION BY CERTIFICATE Certificate_test;
	 IF @TenDangNhap LIKE '%UNION%' OR @MatKhau LIKE '%UNION%'
    BEGIN
        RAISERROR('Tên đăng nhập hoặc mật khẩu không hợp lệ.', 16, 1);
        RETURN;
    END

    -- Kiểm tra đầu vào có chứa chuỗi độc hại
    IF CHARINDEX('=', @TenDangNhap) > 0 OR CHARINDEX('=', @MatKhau) > 0
    BEGIN
        RAISERROR (N'Tên đăng nhập hoặc mật khẩu không hợp lệ.', 16, 1);
        RETURN;
    END
    ...
    -- Các kiểm tra khác để phát hiện SQL Injection
    IF @TenDangNhap LIKE N'%1=1%' OR @MatKhau LIKE N'%1=1%'
    BEGIN
        RAISERROR (N'Tên đăng nhập hoặc mật khẩu không hợp lệ.', 16, 1);
        RETURN;
    END

    -- Kiểm tra các ký tự đặc biệt khác
    IF @TenDangNhap LIKE N'%--%' OR @TenDangNhap LIKE N'%=%' OR @TenDangNhap LIKE N'%''%' OR @TenDangNhap LIKE N'%/*%' OR @TenDangNhap LIKE N'%;%'
    BEGIN
        RAISERROR (N'Tên đăng nhập hoặc mật khẩu không hợp lệ.', 16, 1);
        RETURN;
    END

    IF @MatKhau LIKE N'%--%' OR @MatKhau LIKE N'%=%' OR @MatKhau LIKE N'%''%' OR @MatKhau LIKE N'%/*%' OR @MatKhau LIKE N'%;%'
    BEGIN
        RAISERROR (N'Tên đăng nhập hoặc mật khẩu không hợp lệ.', 16, 1);
        RETURN;
    END
    ...
    IF EXISTS (
        SELECT 1
        FROM NhanVien
        WHERE MaNV = @TenDangNhap AND CONVERT(char(9), DecryptByKey(MatKhau_MH)) = @MatKhau
    )
    BEGIN
    ...
    ELSE
    BEGIN
        PRINT N'Tên đăng nhập hoặc mật khẩu không hợp lệ.';
    END
	CLOSE SYMMETRIC KEY SymKey_test;
END;
----Test THL--
EXEC spDangNhap @TenDangNhap = 'NV001'' OR 1=1 --', @MatKhau = 'mật khẩu giả';
--- Test ok
exec spDangNhap
@TenDangNhap=N'NV_000010',
@MatKhau=N'NK7386344'
